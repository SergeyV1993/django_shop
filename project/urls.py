...
urlpatterns = [
    ...
    re_path(r'^change_password/$',
            PasswordChangeView.as_view(
                ...
                success_url=reverse_lazy('change_password_done')), name='change_password'
            ),

    re_path(r'^change_password_done/$',
            PasswordChangeDoneView.as_view(
                template_name='password/change_password_done.html'), name='change_password_done'
            ),
    ...
    re_path(r'^reset_password_done/$',
            PasswordResetDoneView.as_view(
                template_name='password/reset_password_done.html'), name='reset_password_done'
            ),
    ...
    re_path(r'^reset_password_confirmation/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$',
            PasswordResetConfirmView.as_view(
                ...
                success_url=reverse_lazy('reset_password_confirm_done')), name='reset_password_confirm'
            ),

    re_path(r'^reset_password_confirmation_done/$',
            PasswordResetCompleteView.as_view(
                template_name='password/reset_password_confirmation_done.html'), name='reset_password_confirm_done'
            ),
